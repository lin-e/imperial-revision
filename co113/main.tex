    ...
    \section*{CO113 - Architecture}
            ...
            \subsubsection*{Arrays in C}
                ...
                \begin{center}
                    ...
                    \begin{tabular}{l|l|l}
                        ...
                        ~val + 1~ & ~int*~ & ~x + 4~ \\
                        ~\&val[2]~ & ~int*~ & ~x + 8~ \\
                        ~val[5]~ & ~int~ & ~??~ depends on what is at address ~x+5~\\
                    \end{tabular}
                \end{center}
        ...
        \subsection*{Caches}
            ...
            \subsubsection*{Cache Organisation}
                ...
                \end{itemize}
                It's also important for us to consider writing to, and from the cache, not just reading. The main problem with writing is that the data between the heirachies may disagree with each other. On a write-hit, the possible options are to \textbf{write-through} it writes immediately to memory, or to \textbf{write-back}, it writes to the cache, and defers the write until the line is kicked out. This requires a \textbf{dirty bit} to indicate that the line is different from memory.
                \medskip
