    ...
    \section*{CO202 - Algorithms II}
        ...
        \subsection*{26th November 2019}
            \subsubsection*{Simple PageRank}
                A simple implementation of PageRank is the sum of the SimplePageRanks of the pages pointing to a particular node divided by the number of nodes it's pointing to.
                \begin{center}
                    \begin{tikzpicture}[x=1.5cm]
                        \node[state, minimum size=1cm] (p0) at (0, 0) {$p_0$};
                        \node[state, minimum size=1cm] (p1) at (-1.5, -2) {$p_1$};
                        \node[state, minimum size=1cm] (p2) at (0, -2.5) {$p_2$};
                        \node[state, minimum size=1cm] (p3) at (1.5, -2) {$p_3$};
                        \draw
                        (p0) edge[->, bend left=15] (p1)
                        (p0) edge[->, bend left=15] (p2)
                        (p0) edge[->, bend left=15] (p3)
                        (p1) edge[->, bend left=15] (p0)
                        (p2) edge[->, bend left=15] (p0)
                        (p2) edge[->, bend left=15] (p3)
                        (p3) edge[->, bend left=15] (p0);
                    \end{tikzpicture}
                \end{center}
                \begin{align*}
                    SPR(p_0) & = \frac{SPR(p_1)}{1} + \frac{SPR(p_2)}{2} + \frac{SPR(p_3)}{1} \\
                    SPR(p_1) & = \frac{SPR(p_0)}{3} \\
                    SPR(p_2) & = \frac{SPR(p_0)}{3} \\
                    SPR(p_3) & = \frac{SPR(p_0)}{3} + \frac{SPR(p_1)}{2}
                \end{align*}
                Let $S(p_i)$ be the sources of $p_i$ (what links to it), and $T(p_i)$ be the targets of $p_i$ (where it links to);
                $$SPR(p_i) = \summation{p_j \in S(p_i)}{} \frac{SPR(p_j)}{| T(p_j) |}$$
            \subsubsection*{PageRank}
                In reality, there needs to be some damping factor, as a user may stop clicking at some point.
                Additionally, a user may reach a page from elsewhere, and not just from clicking links.
                $$PR(p_i) = \frac{1 - d}{n} + d \summation{p_j \in S(p_i)}{} \frac{PR(p_j)}{| T(p_j) |}$$
                Note that $n$ is the total number of pages in consideration, and we assume that all pages have outgoing links.
                Since PageRank is a probability distribution, all pages start with $PR(p_i) = \frac{1}{n}$, and this probability gets moved around.
                To avoid making the assumption, we can say that the page rank is distributed if it has no outgoing links;
                $$PR(p_i) = \frac{1 - d}{n} + d \summation{p_j \in S(p_i)}{} \frac{PR(p_j)}{L(p_j)}\text{ where } L(p_j) = \begin{cases}
                    | T(p_j) | & \text{if } | T(p_j) | > 0 \\
                    \frac{1}{n} & \text{otherwise}
                \end{cases}$$
        \subsection*{29th November 2019}
            \subsubsection*{Ford-Fulkerson Max-Flow Method}
                In the graph below, we have the maximum capacity on each edge.
                There is a source node $s$, and a target node $t$.
                \begin{center}
                    \begin{tikzpicture}
                        \node[state, minimum size=1cm] (s) at (0, 0) {$s$};
                        \node[state, minimum size=1cm] (v1) at (2, 1) {$v_1$};
                        \node[state, minimum size=1cm] (v2) at (2, -1) {$v_2$};
                        \node[state, minimum size=1cm] (v3) at (4, 1) {$v_3$};
                        \node[state, minimum size=1cm] (v4) at (4, -1) {$v_4$};
                        \node[state, minimum size=1cm] (t) at (6, 0) {$t$};
                        \draw
                        (s) edge[->, above] node{16} (v1)
                        (s) edge[->, below] node{13} (v2)
                        (v1) edge[->, above] node{12} (v3)
                        (v2) edge[->, right] node{4} (v1)
                        (v2) edge[->, below] node{14} (v4)
                        (v3) edge[->, below] node{9} (v2)
                        (v3) edge[->, above] node{20} (t)
                        (v4) edge[->, right] node{7} (v3)
                        (v4) edge[->, below] node{4} (t);
                    \end{tikzpicture}
                \end{center}
                This starts by picking an augmenting path, which is any path from $s$ to $t$.
                Once we pick the path, we add the minimum capacity of all the capacities on the path to the "flow used".
                We keep doing this until we cannot pick more paths - since the next path that is picked isn't exact, this is a method and not an algorithm.
                \medskip

                An implementation (not working) of this in Haskell is as follows;
                \begin{lstlisting}
                    untilNothing :: (a -> Maybe b) -> (b -> a -> a) -> a -> a
                    -- (a -> Maybe a) takes a graph, and may produce a path
                    -- (b -> a -> a)  takes a path, a graph, and gives back a graph
                    -- a              is the initial graph
                    -- a              is the final graph
                    ff g s t = untilNothing (findPath s t) augment (initGraph g)

                    findPath :: v -> v -> g -> Maybe Path
                    findPath s t g = bfs -- (Edmonds Karp)

                    augment :: Path -> Graph -> Graph
                \end{lstlisting}
                Note that we could also pass in the ~findPath~ function, since changing that for Ford Fulkerson changes the algorithm.
                The idea here is that some algorithms can be comprised of smaller algorithms, which can change its complexity, and similarly with data structures.
\end{document}