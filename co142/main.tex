    ...
    \section*{CO142 - Discrete Structures}
        ...
        \subsection*{9th October 2018}
            ...
            \subsubsection*{Logical Formula, and Notation}
                ...
                \begin{itemize}
                    ...
                    \item $a \in A$ \hfill the object $a$ is in the set $A$ ($a$ is an element of $A$)
        ...
        \subsection*{12th October 2018}
            ...
            \subsubsection*{Set Composition}
                \begin{itemize}
                    ...
                    \item $A \cap B = \emptyset$ \hfill $A$, and $B$ are disjoint sets
                \end{itemize}
        ...
        \subsection*{16th October 2018}
            ...
            \subsubsection*{Operator Properties}
                ...
                \begin{itemize}
                    \itemsep0em
                    \item $A \cup A = A$ \hfill idempotence
                    \item $A \cap A = A$ \hfill idempotence
                ...
                \end{itemize}
                Note that we are able to use the properties of logical connectives to aid us in our proofs, since those are fairly easy to prove with truth tables, as they have a finite number of configurations. For example, the proof of idempotence inherently uses the property $p \land p \equiv p$, and the same for $\lor$.
        ...
        \subsection*{30th October 2018}
            ...
            \subsubsection*{Transitive Closure}
                ...
                Consider some relation $<_1 \triangleq \{\la m, n \ra \in \mathbb{N}^2 \mid m = n + 1\}$ over $\mathbb{N}$. This is relation represents a difference of 1. Now, we can construct $<_2 = <_1 \circ <_1$ (fairly trivial to prove, since we know it covers the naturals, we can almost approach it inductively, however there is an easier solution where we use the properties of natural numbers (hint - if $n \in \mathbb{N}$, then $n + 1 \in \mathbb{N}$)). The transitive closure on $<_1$, $<_1^+$, is therefore $<$.
                \medskip
        ...
        \subsection*{6th November 2018}
            \subsubsection*{Functions}
                We define a function $f$, from $A$ (the function domain to $B$ (the function co-domain) as $f : A \to B$. \textbf{Every} element of $A$ must map to a \textbf{unique} (exactly 1) element in $B$. We can formally write this as follows, where both conditions must hold;
                \begin{enumerate}[1.]
                    \itemsep0em
                ...
                \begin{itemize}
                    \itemsep0em
                    \item $f : A \to B$ is short for $f \subseteq B^A$
                    \item $\exists f : A \to B [\cdots]$ is short for $\exists f \in B^A [...]$
                    \item $\forall f : A \to B [\cdots]$ is short for $\forall f \in B^A [...]$
                    \item suppose that $A$ is an $n$-ary product $A_1 \times ... \times A_n$, we can write $f(a_1, ..., a_n)$ instead of $f(\la a_1, ..., a_n \ra)$
                \end{itemize}
                We define equality on two functions $f, g : A \to B$, as $f =_{A \times B} g \triangleq \forall x \in A [f(x) =_B g(x)]$.
                \medskip
                ...
                \begin{itemize}
                    \itemsep0em
                    \item $f : \mathbb{N}^2 \to \mathbb{N}$ \hfill $f(x, y) = x + y$
                    \item $f : \mathbb{N} \to \mathbb{N}$ \hfill $f(x) = x^2$
                    \item $f : \mathbb{R} \to \mathbb{R}$ \hfill $f(x) = x + 3$
                \end{itemize}
            ...
            \subsubsection*{Characteristic Function}
                Suppose we have a set $A$, and the characteristic function of $B \subseteq A$ is defined as $\chi_B : A \to \{0, 1\}$, and for some $n$-ary relation $R$, such that $R \subseteq A_1 \times ... \times A_n$, we can define $\chi_R : A_1 \times ... \times A_n \to \{0, 1\}$;
                \begin{align*}
                    \chi_B(a) & =
                    \begin{cases}
                        1 & \text{if } a \in B \\
            ...
            \subsubsection*{Partial Functions}
                A partial function is a function \textbf{without} the second condition, therefore not all elements in $A$, must have a corresponding element in $B$. We denote an undefined value as $\bot$, therefore we can create a "function" from a partial function by saying $f : A \to (B \cup \{\bot\})$. If you are asked to give a function in an exam, \textbf{do not give a partial function}.
            \subsubsection*{Properties of Functions}
                We will be working on some function $f : A \to B$.
                \begin{itemize}
                    \itemsep0em
                ...
                \end{itemize}
                The (Dual) Cantor-Bernstein Theorem states that if there exists $f : A \to B$, and $g : B \to A$, where they are both surjective, or both injective, then it follows that there is is a bijection $h : A \to B$. This theorem is extremely helpful when we want to prove that two infinite sets have the same cardinality.
                \medskip
                ...
                \begin{itemize}
                    ...
                    \item $f : \mathbb{R} \to \mathbb{R}$ \hfill $f(x) = 4x + 3$
                        \subitem this is bijective
                \end{itemize}
            \subsubsection*{Some Proof on Function Image?}
                No idea what this should actually be titled. But let there be a finite set $A$, a function $f : A \to B$, and $X \subseteq A$. We want to prove that $|f[X]| \leq |X|$.
                \medskip

                By contradiction, let us assume that $|f[X]| > |X|$. Define a function $p$, such that $p : f[X] \to X$. Suppose we pick an arbitrary $b \in f[X]$, and some $a \in X$ (we know this exists by definition of image), such that we have $f(a) = b$, and define $p(b) = a$.
                \medskip
        ...
        \subsection*{9th November 2018}
            ...
            \subsubsection*{Proposition on the Property of Functions}
                Given two \textbf{finite} sets (these do not always apply to infinite sets) $A$, and $B$, and a function $f : A \to B$, we can say that if...
                \begin{itemize}
                    \itemsep0em
            ...
            \subsubsection*{Function Composition}
                Suppose we have arbitrary sets, $A$, $B$, $C$, and let there be two functions $f : A \to B$, and $g : B \to C$. We can define the composition of $f$ with $g$ (meaning $g$ applied to the image of $f$), as $\la a, c \ra \in (g \circ f) \triangleq \exists b \in B [\la a, b \ra \in f \land \la b, c \ra \in g]$ where $(g \circ f) : A \to C$. It's important to note that the order of the arguments is flipped, compared to relation composition. The crucial requirement is that the co-domain of $f$ matches the domain of $g$.
                \medskip

                We can prove that function composition is associative quite trivially. Recall that two functions $i, j$ (let them both be functions from $A$ to $B$) are equal if $\forall a \in A [i(a) =_B j(b)]$. We want to prove that $h \circ (g \circ f) = (h \circ g) \circ f$, and we are using the same definitions mentioned previously, plus a new set $D$, and $h : C \to D$. We first take an arbitrary $a \in A$;
                \begin{align*}
                ...
                When we're working with function compositions, especially when we're told to give a specific example, it's useful to draw out diagrams (wish I knew that during the Christmas exam). The example below has $f : A \to B$, such that $f = \{\la a_1, b_1 \ra, \la a_2, b_2 \ra\}$, and $g : B \to C$, such that $g = \{\la b_1, c_1 \ra, \la b_2, c_1 \ra, \la b_3 c_2 \ra\}$. Thus the composed function $(g \circ f) : A \to B$, is $g \circ f = \{\la a_1, c_1 \ra, \la a_2, c_2 \ra\}$.
                \begin{center}
                    \begin{tikzpicture}
                        \node[] () at (0, 2.5) {$A$};
                        \node[] () at (5, 2.5) {$B$};
            ...
            \subsubsection*{Proofs on Properties of Composed Functions}
                ...
                We want to show that if we have bijections $f : A \to B$, and $g : B \to C$, then $g \circ f$ is a bijection. It is sufficient to show that if $f, g$ are both surjective, then so is $g \circ f$, and if $f, g$ are both injective, then so is $g \circ f$.
                \medskip
            ...
            \subsubsection*{Cardinality of Sets}
                ...
                $A \approx B \triangleq \exists f : A \to B$ (where $f$ is a bijection). Note that the previously mentioned (Dual) Cantor-Bernstein Theorem states that there is a bijection when there are two injective (or two surjective) functions $g : A \to B$, and $h : B \to A$.
                \medskip
                ...
                \begin{itemize}
                    ...
                    \item $\approx$ is reflexive, hence $A \approx A$
                        \subitem we have the identity relation $\text{id}_A : A \to A$, which is a bijection
                    \item $\approx$ is symmetric, hence if $A \approx B$, then $B \approx A$
                        \subitem given that there is a bijection $f : A \to B$, there exists an inverse $f^{-1} : B \to A$, which is also a bijection (proven above)
                    \item $\approx$ is transitive, hence if $A \approx B$, and $B \approx C$, then $A \approx C$
                        \subitem given that there exists two bijections, $f : A \to B$, and $g : B \to C$, the composition $(g \circ f) : A \to C$ is also a bijection (proven above)
                \end{itemize}
                ...
                \begin{center}
                    ...
                    \end{tikzpicture}
                \end{center}
            \subsubsection*{Example of Bijection}
                At the start of the course, we considered the Cartesian product of sets, and considered whether it was associative. While they are not equal, we can build a bijection between them. For example, $f : (A \times B) \times C \to A \times (B \times C)$. In order to precisely define this, without using Haskell-style pattern matching, we can define $L(x, y) = x$, and $R(x, y) = y$. Therefore, to get $f(\la a, b \ra, c) = (a, \la b, c \ra)$, we define $f(x, y) = (L(x), \la R(x), y \ra)$, and similar for the inverse.
        \subsection*{13th November 2018}
            \subsubsection*{Cardinality of Arbitrary Sets}
                We previously defined the cardinality of a finite set as the number of elements in it. Consider arbitrary finite sets $A$, and $B$, such that $|A| = |B| = n$. It is then trivial for us to construct a bijection $c_A : \{1, 2, ..., n\} \to A$, and similarly $c_B : \{1, 2, ..., n\} \to B$. Because the former is a bijection, we have a corresponding inverse function $c_A^{-1} : A \to \{1, 2, ..., n\}$. Now, we can therefore form a third bijection $(c_A^{-1} \circ c_B) A \to B$, hence $A \approx B$.
            \subsubsection*{Cantor's Theorem}
                ...
                To do this, we will do a proof by contradiction. Assume that there exists some bijection (therefore we also get the fact that it's surjective without doing any work) $f : A \to \powerset A$. Define a set $B = \{x \in A \mid x \notin f(x)\}$. Note that this has a clear similarity to Russel's paradox. Now, it's clear that $B \subseteq A$, since we're constructing from a well defined set. Therefore, it follows that $B \in \powerset A$, by the definition of $\powerset$. However, from our assumption, we're given that it's a surjection, hence there exists some $b \in A$, such that $f(b) = B$.
                \medskip

                We then get $b \in f(b)$ (same as $b \in B$), or $b \notin f(b)$. Consider the first case, when $b \in f(b) = B$, therefore, it means it satisfies the criteria that $b \notin f(b)$, thus $\lightning$. Consider the latter case, where $b \notin f(b)$, by the definition of $B$, $b \in B$, therefore $b \in f(b)$, hence $\lightning$. Because we get a contradiction on both cases, the assumption leads to falsity, hence $f$ is not surjective, therefore it definitely isn't bijective. As there doesn't exist a bijection between the two sets, they do not have the same cardinality.
                \medskip
            ...
            \subsubsection*{Countable Sets}
                We can say some set $A$ countable if it is finite, or $A \approx \mathbb{N}$. The elements of a countable set can be listed as a \textbf{finite, or infinite} sequence of distinct terms $A = \{a_1, a_2, a_3, ...\}$. For example, we can write the integers as $\mathbb{Z} = \{0, -1, 1, -2, 2, -3, 3, ...\}$, where the "counting bijection" $g : \mathbb{N} \to \mathbb{Z}$ is defined as follows;
                \begin{align*}
                    g(x) & =
                    \begin{cases}
                ...
                In order to prove that the set $\{0, 1\} \times \mathbb{N}$ is countable, we can either construct a diagram similar to the one for $\mathbb{N}^2$, or we can define a bijection function $g : \mathbb{N} \to \{0, 1\} \times \mathbb{N}$, defined as follows;
                \begin{align*}
                    g(x) & =
                    \begin{cases}
                        \la 0, \frac{x}{2} \ra & x \text{ is even}\\
                        \la 1, \frac{x + 1}{2} \ra & x \text{ is odd}
                    \end{cases}
                \end{align*}
        \subsection*{16th November 2018}
            \subsubsection*{Something on the Union of Countable Sets}
                ...
                \begin{enumerate}[1.]
                    \itemsep0em
                    \item Both $X$, and $Y$ are finite sets. This is the simplest case, as we can say that $|X| = n$, and $|Y| = m$, where $n, m \in \mathbb{N}$, hence $|X \cup Y| = n + m \in \mathbb{N}$, due to the closure of addition on the natural numbers. We can take the sum, as proven at the start of the module.
                    \item $X$ is a finite set, and $Y$ is infinite. As $Y$ is countable, we can say $Y \approx \mathbb{N}$, hence there exists some bijection $f : \mathbb{N} \to Y$. Since $X$ is finite, we can say $|X| = n$, and also $X = \{x_1, x_2, ..., x_n\}$. Let us now construct another bijection, $h : \mathbb{N} \to X \cup Y$, defined as follows;
                        \begin{align*}
                            h(i) & =
                            \begin{cases}
                                x_i & \text{if } i \leq |X| \\
                    ...
                    \item $X$ is an infinite set, and $Y$ is finite. Same proof as above, but with the two sets swapped.
                    \item Both $X$, and $Y$, are countably infinite sets. This proof relies on the fact that $\{0, 1\} \times \mathbb{N}$ is countable. Let us make some bijection $k : \mathbb{N} \to \{0, 1\} \times \mathbb{N}$ defined as;
                        \begin{align*}
                            k(n) & =
                            \begin{cases}
                                \la 0, \frac{n}{2} \ra & n \text{ is even} \\
                                \la 1, \frac{n - 1}{2} \ra & n \text{ is odd}
                            \end{cases}
                        \end{align*}
                        Since they are both countable, it follows that $X \approx \mathbb{N}$, and $Y \approx \mathbb{N}$, hence there exists a pair of bijections $f : \mathbb{N} \to X$, and $g : \mathbb{N} \to Y$. Let us also define an additional $h : \{0, 1\} \times \mathbb{N} \to X \cup Y$, defined as;
                        \begin{align*}
                            h(b, n) & =
                            \begin{cases}
                                f(n) & b = 0 \\
                            ...
                            \end{cases}
                        \end{align*}
                        Due to function composition, we could also create a bijection from $\mathbb{N} \to X \cup Y$, but proving that the Cartesian product is countable should be sufficient.
                \end{enumerate}
            \subsubsection*{Diagonalisation}
                To clarify on the notation used; I am using $v_i^j$ to denote the $i^\text{th}$ element of $V^j$.
                \medskip

                We want to prove that $\powerset \mathbb{N}$ is uncountable, as $\mathbb{N} \not\approx \powerset \mathbb{N}$. Any subset $V \subseteq \mathbb{N}$ can be represented as a list of 0s, and 1s. For example, let the set $V^j$ be represented as $v_0^j, v_1^j, v_2^j, ...$, based on the characteristic function $\chi_{V^j}$, such that $v_i^j = 1$ if $i \in V^j$, otherwise $v_i^j = 0$ if $i \notin V^j$ - it's important to note that $i \in \mathbb{N}$. Suppose there is a set of sets $V^0, V^1, V^2, ...$, let us define a new set $W$, where the $i^\text{th}$ element, $w_i$, is defined as $w_i = 1 - v^i_i$. As we're still bounded to 0s, and 1s - it follows that $W \subseteq \mathbb{N}$. From the definition of $W$, we get that $\forall i \in \mathbb{N} [i \in W \Leftrightarrow i \notin V^i]$ therefore this new set is different from every other set $V$, in at least one item (hence $\forall i \in \mathbb{N} [W \neq V^i]$). The diagram below visualises this concept;
                \begin{center}
                    \begin{tikzpicture}
                        \foreach \x in {0,...,4} {
                            \foreach \y in {0,...,4} {
                                \node[] () at (1.5*\x, -\y) {$v^\y_\x$};
                            }
                        }
                        \foreach \x in {0,...,4} {
                            \node[] () at (1.5*\x, -5) {$\vdots$};
                            \node[] () at (1.5*\x, -6) {$1 - v_\x^\x$};
                            \draw
                            (1.5*\x, -\x) ellipse (0.5 and 0.5);
                        }
                        \foreach \y in {0,...,4} {
                            \node[] () at (7.5, -\y) {$\cdots$};
                            \node[] () at (-1, -\y) {$=$};
                    ...
                    \end{tikzpicture}
                \end{center}
            \subsubsection*{Proving the Rationals are Insignificant in the Reals}
                Here, we want to show that $\mathbb{Q}$ is insignificant (hence a zero-set) in $\mathbb{R}$. Since we've proven that the rationals are countable, we can write $\mathbb{Q} = \{q_0, q_1, q_2, ...\}$. We can construct intervals in $\mathbb{R}$, $V_\delta^i \subseteq \mathbb{R}$ as $V_\delta^i \subseteq \mathbb{R}$, as $V_\delta^i \triangleq (q_i - \delta \cdot 2^{-i}, q_i + \delta \cdot 2^{-i})$. Therefore, we can say that each interval therefore has a size of $\delta \cdot 2^{1-i}$, which can be written as $||V_\delta^i||$, for brevity.
                \smallskip

                We can also say that $V_\delta \triangleq \bigcup\limits_{i = 0}^\infty V_\delta^i$, hence $\mathbb{Q} \subseteq V_\delta$. As well as $0 < ||V_\delta|| \leq \summation{i = 0}{\infty}{||V_\delta^i||} = 2\delta\summation{i = 0}{\infty}{2^{-i}} = 4\delta$.
                \smallskip

                We've therefore defined that the size of $V_\delta$ is bounded between 0, and $4\delta$. Since this is true for any $\delta$, if we take $V = \limit{\delta}{0}V_\delta$, we approach 0, we show that $V$ is negligible. Since $\mathbb{Q} \subseteq V$, $\mathbb{Q}$ is a null set in $\mathbb{R}$.
            \subsubsection*{Orderings}
                Define $R$ as a binary relation on $A$, we can then say the following properties;
                \begin{itemize}
                    \itemsep0em
                    \item $R$ is a pre-order \hfill $R$ is reflexive, and transitive
                    \item $R$ is anti-symmetric \hfill $\forall a, b \in A [a\ R\ b \land b\ R\ a \Rightarrow a =_A b]$
                    \item $R$ is a partial order (often written as $\leq$) \hfill $R$ is reflexive, transitive, and anti-symmetric
                        \subitem $(A, \leq)$, or $\leq_A$ means a partial order $\leq$ on $A$
                    \item $R$ is irreflexive \hfill $\forall a \in A [\neg (a\ R\ a)]$
                    \item $R$ is a strict partial order (often written as $<$) \hfill $R$ is irreflexive, and transitive
                    \item $R$ is a total order \hfill a partial order that satisfies $\forall a, b \in A [a\ R\ b \lor b\ R\ a]$
                \end{itemize}
                Given two partial orders $(A, \leq_A)$, and also $(B, \leq_B)$, the important orders on the product set $P = A \times B$ are;
                \begin{itemize}
                    \itemsep0em
                    \item product order \hfill $\la a_1, b_1 \ra \leq_P \la a_2, b_2 \ra \triangleq a_1 \leq_A a_2 \land b_1 \leq_B b_2$
                    \item lexicographical order \hfill $\la a_1, b_1 \ra \leq_P \la a_2, b_2 \ra \triangleq a_1 \leq_A a_2 \lor (a_1 =_A a_2 \land b_1 \leq_B b_2)$
                \end{itemize}
            ...
            \subsubsection*{Hasse Diagrams}
                By convention, Hasse diagrams only record the immediate predecessors (as the rest can be inferred). The direction of the lines are often omitted, and conventionally are read by looking up the page. For example, the Hasse diagrams for the binary relation 'divides' on the set $\{1, 2, 3, 6, 12, 18\}$, and the binary relation $\subseteq$ on $\powerset \{\alpha, \beta, \gamma\}$ are as follows;
                \begin{center}
                    \begin{tikzpicture}
                        \node[circle, fill=black, inner sep=1.5pt, label=below:{1}] (1) at (-6, 0) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{2}] (2) at (-8, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=right:{3}] (3) at (-4, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=below:{6}] (6) at (-6, 2) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{12}] (12) at (-8, 3) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=right:{18}] (18) at (-4, 3) {};
                        \draw
                        (1) -- (2)
                        (1) -- (3)
                        (2) -- (6)
                        ...
                        \node[circle, fill=black, inner sep=1.5pt, label=below:{$\emptyset$}] (e) at (0, 0) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{$\{\alpha\}$}] (a) at (-2, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{$\{\beta\}$}] (b) at (0, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=right:{$\{\gamma\}$}] (g) at (2, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{$\{\alpha, \beta\}$}] (ab) at (-2, 2) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{$\{\alpha, \gamma\}$}] (ag) at (0, 2) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=right:{$\{\beta, \gamma\}$}] (bg) at (2, 2) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=right:{$\{\alpha, \beta, \gamma\}$}] (abg) at (0, 3) {};
                        \draw
                        (e) -- (a)
                        (e) -- (b)
                        (e) -- (g)
                    ...
                    \end{tikzpicture}
                \end{center}
            \subsubsection*{Properties of Partial Orders}
                Suppose there is a partial order $(A, \leq)$, and $a \in A$;
                \begin{itemize}
                    \itemsep0em
                    \item $a$ is minimal \hfill $\forall b \in A [b \leq_A a \Rightarrow b =_A a]$
                    \item $a$ is maximal \hfill $\forall b \in A [a \leq_A b \Rightarrow b =_A a]$
                    \item $a$ is least \hfill $\forall b \in A [a \leq_A b]$
                    \item $a$ is greatest \hfill $\forall b \in A [b \leq_A a]$
                \end{itemize}
                ...
                By using logical equivalences, we can derive the condition for $a$ to be minimal in a \textbf{strict partial order} $(A, <)$, note that equality is no longer considered, since we have irreflexivity);
                \begin{align*}
                    \forall b \in A [b \leq_A a \Rightarrow b =_A a] & \equiv \forall b \in A [\neg (b \leq_A a) \lor b =_A a] \\
                    & \equiv \forall b \in A [\neg (b \leq_A a \land b \neq_A a)] \\
                    & \equiv \neg \exists b \in A [b \leq_A a \land b \neq_A a] \\
                    & \equiv \neg \exists b \in A [b <_A a]
                \end{align*}
        \subsection*{20th November 2018}
            \subsubsection*{Questions on the Divisor Order}
                The following questions concern a divisor order $\leq_D$ defined on the positive natural numbers ($\mathbb{N}^+$), defined such that $n \leq_D m \triangleq n \text{ divides } m$ (or more formally as $\exists p \in \mathbb{Z} [m = p \cdot n]$).
                \medskip
                ...
                \begin{center}
                    \begin{tikzpicture}
                        \node[circle, fill=black, inner sep=1.5pt, label=below:{2}] (2) at (-1, 0) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=below:{5}] (5) at (1, 0) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=below:{10}] (10) at (0, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=left:{4}] (4) at (-2, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=right:{25}] (25) at (2, 1) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=above:{12}] (12) at (-2, 2) {};
                        \node[circle, fill=black, inner sep=1.5pt, label=above:{20}] (20) at (0, 2) {};
                        \draw
                        (2) -- (4)
                        (2) -- (10)
                        (5) -- (10)
            ...
            \subsubsection*{Well-founded Partial Orders}
                A well-founded partial order is defined as a partial order that has no infinite \textbf{decreasing} chain of elements. For every infinite sequence, written as $a_1, a_2, a_3, ...$, such that $a_1 \geq a_2 \geq ...$, there exists some point $m \in \mathbb{N}$, such that $a_n = a_m$ for any $n \geq m$.
                \medskip

                If we have two well-founded partial orders $(A, \leq_A)$, and $(B, \leq_B)$, then the lexicographical order of $L = A \times B$ is also well-founded. By contradiction, let us consider a chain $\la a_1, b_1 \ra \geq_L \la a_2, b_2 \ra \geq_L \la a_3, b_3 \ra \geq_L ...$. Then, by lexicographical ordering, it follows that $a_1 \geq_A a_2 \geq_A a_3 \geq_A ...$, until some point, where it's just repeating $a_m$ (since we're told it's well-founded). After this point, since $a_n = a_m$ for all $n \geq m$, we start checking $b$ values. Now, we have $b_m \geq_B b_{m + 1} \geq_B b_{m + 2} \geq_B b_{m + 3} \geq_B ...$,to another limit, let it be $r$, where it continues to repeat values. As we now have repeating $\la a_r, b_r \ra$, it follows that the lexicographical ordering must also be well-founded.
            \subsubsection*{Ackermann Function}
                We can prove that the Ackermann function (will be referred to as $A$ for brevity) terminates. This relies on the previous proof that the lexicographical ordering of the product set is also well-founded if the 'factors' are. Note that the Ackermann function is defined as $A : \mathbb{N}^\to \mathbb{N}$, and concerns the following cases;
                \begin{align*}
                    A(0, y) & = y + 1 \\
                    A(x + 1, 0) & = A(x, 1) \\
                    A(x + 1, y + 1) & = A(x, A(x + 1, y))
                \end{align*}
                We define the lexicographical ordering of natural number pairs as $\la x, y \ra < \la x^\prime, y^\prime \ra \triangleq x < x^\prime \lor (x = x^\prime \land y < y^\prime)$. Applying this to the inputs of the functions, we obtain the following results;
                \begin{align*}
                    \la x + 1, 0 \ra & > \la x, 1 \ra \\
                    \la x + 1, y + 1 \ra & > \la x, A(x + 1, y) \ra \\
                    \la x + 1, y + 1 \ra & > \la x + 1, y \ra
                \end{align*}
                Therefore, the function must terminate.
        \subsection*{Induction}
            Refer to \textbf{CO141 - Reasoning About Programs} for a detailed introduction to induction, including mathematical, structural, as well as strong induction.
\end{document}