    ...
    \section*{CO142 - Discrete Structures}
        ...
        \subsection*{6th November 2018}
            ...
            \subsubsection*{Properties of Functions}
                ...
                \end{itemize}
                The (Dual) Cantor-Bernstein Theorem states that if there exists $f : A \mapsto B$, and $g : B \mapsto A$, where they are both surjective, or both injective, then it follows that there is is a bijection $h : A \mapsto B$. This theorem is extremely helpful when we want to prove that two infinite sets have the same cardinality.
                \medskip
        ...
        \subsection*{9th November 2018}
            ...
            \subsubsection*{Example of Bijection}
                At the start of the course, we considered the Cartesian product of sets, and considered whether it was associative. While they are not equal, we can build a bijection between them. For example, $f : (A \times B) \times C \mapsto A \times (B \times C)$. In order to precisely define this, without using Haskell-style pattern matching, we can define $L(x, y) = x$, and $R(x, y) = y$. Therefore, to get $f(\la a, b \ra, c) = (a, \la b, c \ra)$, we define $f(x, y) = (L(x), \la R(x), y \ra)$, and similar for the inverse.
        \subsection*{13th November 2018}
            \subsubsection*{Cardinality of Arbitrary Sets}
                We previously defined the cardinality of a finite set as the number of elements in it. Consider arbitrary finite sets $A$, and $B$, such that $|A| = |B| = n$. It is then trivial for us to construct a bijection $c_A : \{1, 2, ..., n\} \mapsto A$, and similarly $c_B : \{1, 2, ..., n\} \mapsto B$. Because the former is a bijection, we have a corresponding inverse function $c_A^{-1} : A \mapsto \{1, 2, ..., n\}$. Now, we can therefore form a third bijection $(c_A^{-1} \circ c_B) A \mapsto B$, hence $A \approx B$.
            ...
            \subsubsection*{Countable Sets}
                ...
                In order to prove that the set $\{0, 1\} \times \mathbb{N}$ is countable, we can either construct a diagram similar to the one for $\mathbb{N}^2$, or we can define a bijection function $g : \mathbb{N} \mapsto \{0, 1\} \times \mathbb{N}$, defined as follows;
                \begin{align*}
                    g(x) & =
                    \begin{cases}
        ...
        \subsection*{16th November 2018}
            \subsubsection*{Something on the Union of Countable Sets}
                ...
                \begin{enumerate}[1.]
                    ...
                    \item $X$ is a finite set, and $Y$ is infinite. As $Y$ is countable, we can say $Y \approx \mathbb{N}$, hence there exists some bijection $f : \mathbb{N} \mapsto Y$. Since $X$ is finite, we can say $|X| = n$, and also $X = \{x_1, x_2, ..., x_n\}$. Let us now construct another bijection, $h : \mathbb{N} \mapsto X \cup Y$, defined as follows;
                        \begin{align*}
                            h(i) & =
                            \begin{cases}
                                x_i & \text{if } i \leq |X| \\
                    ...
                    \item Both $X$, and $Y$, are countably infinite sets. This proof relies on the fact that $\{0, 1\} \times \mathbb{N}$ is countable. Let us make some bijection $k : \mathbb{N} \mapsto \{0, 1\} \times \mathbb{N}$ defined as;
                        \begin{align*}
                            k(n) & =
                            \begin{cases}
                            ...
                            \end{cases}
                        \end{align*}
                        Since they are both countable, it follows that $X \approx \mathbb{N}$, and $Y \approx \mathbb{N}$, hence there exists a pair of bijections $f : \mathbb{N} \mapsto X$, and $g : \mathbb{N} \mapsto Y$. Let us also define an additional $h : \{0, 1\} \times \mathbb{N} \mapsto X \cup Y$, defined as;
                        \begin{align*}
                            h(b, n) & =
                            \begin{cases}
                                f(n) & b = 0 \\
                            ...
                            \end{cases}
                        \end{align*}
                        Due to function composition, we could also create a bijection from $\mathbb{N} \mapsto X \cup Y$, but proving that the Cartesian product is countable should be sufficient.
                \end{enumerate}
