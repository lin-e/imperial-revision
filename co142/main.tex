    ...
    \section*{CO142 - Discrete Structures}
        ...
        \subsection*{16th November 2018}
            ...
            \subsubsection*{Proving the Rationals are Insignificant in the Reals}
                Here, we want to show that $\mathbb{Q}$ is insignificant (hence a zero-set) in $\mathbb{R}$. Since we've proven that the rationals are countable, we can write $\mathbb{Q} = \{q_0, q_1, q_2, ...\}$. We can contruct intervals in $\mathbb{R}$, $V_\delta^i \subseteq \mathbb{R}$ as $V_\delta^i \subseteq \mathbb{R}$, as $V_\delta^i \triangleq (q_i - \delta \cdot 2^{-i}, q_i + \delta \cdot 2^{-i})$. Therefore, we can say that each interval therefore has a size of $\delta \cdot 2^{1-i}$, which can be written as $||V_\delta^i||$, for brevity.
                \smallskip
                ...
                We've therefore defined that the size of $V_\delta$ is bounded between 0, and $4\delta$. Since this is true for any $\delta$, if we take $V = \limit{\delta}{0}V_\delta$, we approach 0, we show that $V$ is neglgiible. Since $\mathbb{Q} \subseteq V$, $\mathbb{Q}$ is a null set in $\mathbb{R}$.
