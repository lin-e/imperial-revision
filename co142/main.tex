    ...
    \section*{CO142 - Discrete Structures}
        ...
        \subsection*{16th November 2018}
            ...
            \subsubsection*{Proving the Rationals are Insignificant in the Reals}
                Here, we want to show that $\mathbb{Q}$ is insignificant (hence a zero-set) in $\mathbb{R}$. Since we've proven that the rationals are countable, we can write $\mathbb{Q} = \{q_0, q_1, q_2, ...\}$. We can construct intervals in $\mathbb{R}$, $V_\delta^i \subseteq \mathbb{R}$, as $V_\delta^i \triangleq (q_i - \delta \cdot 2^{-i}, q_i + \delta \cdot 2^{-i})$. Therefore, we can say that each interval therefore has a size of $\delta \cdot 2^{1-i}$, which can be written as $||V_\delta^i||$, for brevity.
                \smallskip
