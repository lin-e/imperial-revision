...
\begin{document}
    ...
    \section*{CO221 - Compilers}
        ...
        \subsection*{17th February 2020 \hfill Tutorial}
            \subsubsection*{Lexical Analysis}
                Note that when we are given an alphabet, we only consider the characters in that alphabet (whereas normally we'd consider the entire character set in actual use).
                Another tip, when dealing with regex meta-characters, is to cast those into standard characters to make it easier to work with, and replace them at the end (escaping if needed).
                \begin{enumerate}[1.]
                    \itemsep0em
                    \setcounter{enumi}{9}
                    \item For the alphabet \{~A~, ~B~\} give a regular expression for all strings containing an even number of ~A~s and an even number of ~B~s.
                        \medskip

                        ~(AA | BB | (AB|BA) (AA|BB)* (AB|BA))*~
                        \smallskip

                        The interesting case is the third (outer) alternative - if there is a string that uses each character once, there must be another string of two at some point that uses each character once.
                    \setcounter{enumi}{14}
                    \item This was a trick question - because we cannot count the number of 0s, and the number of 1s (since regular expressions cannot count).
                    \item The idea here was to see what the cycles in the DFA do (and what is in the accepting state).
                    \item In this question, because it was finite (due to no cycles) - it was possible to enumerate the values it could generate (since it was binary, it made sense to check the values in order).
                \end{enumerate}
            \subsubsection*{LR Parsing}
                In the case of context free grammars, we are able to utilise recursion, which mirrors how programming language structure is defined.
                \begin{enumerate}[1.]
                    \itemsep0em
                    \item
                        This continues from the limitation of regular expressions (question 5 of the last tutorial).
                        We want to obtain the CFG for the set of strings where there are $n$ 0s followed by $n$ 1s.
                        \medskip

                        We can start by looking at what we want to capture; ~01~, ~0011~, ~000111~, and so on.
                        You'll notice that (other than the first case, $n = 1$), the cases build on the case before it, due to the recursive nature, by adding a ~0~ to the start, and a ~1~ afterwards.
                        \begin{center}
                            $~S~ \to ~0 1~ \bnfsep ~0 S 1~$
                        \end{center}
                    \setcounter{enumi}{2}
                    \item
                        In this question, we typically assume that the start symbol is the first rule listed.
                        From that rule, we look at the alternatives to it, and attempt them to find one that fits (looking at the terminals (if possible) is a good way to "guess" the alternative we need).
                    \item Show all binary strings generated in the following grammar are divisible by 3;
                        \begin{center}
                            $~num~ \to ~11~ \bnfsep ~1001~ \bnfsep ~num 0~ \bnfsep ~num num~$
                        \end{center}
                        This is done via numerical values for nodes in the parse tree - going through each alternative;
                        \begin{enumerate}[rule 1:]
                            \itemsep0em
                            \item ~11~ is decimal 3, and is trivially divisible by 3
                            \item ~1001~ is decimal 9, and is trivially divisible by 3
                            \item ~num 0~ (from here, we use induction and assume ~num~ is divisible by 3) - this multiplies ~num~ by 2, and if ~num~ is a multiple of 3, the new number also remains as a multiple of 3
                            \item ~num num~ (we assume both ~num~s are multiples of 3) - let the length of the second ~num~ be $k$ bits, and therefore this multiplies the first ~num~ by $2^k$, and adds the second ~num~ - multiplying keeps it as a multiple of 3, and adding a  multiple of 3 to a multiple of 3 is a multiple of 3
                        \end{enumerate}
                    \setcounter{enumi}{11}
                    \item Note that we check whether the grammar is LR(1) by looking at the constructed parsing table - if it has any conflicts, then it cannot be LR(1).
                        \begin{center}
                            $~C~ \to ~C i o~ \bnfsep ~i o~$
                        \end{center}
                        Recall that we need to add an auxiliary state, $~C'~ \to ~C \$~$.
                        When we have a non-terminal in a state, being the "next" symbol toe read, we can compute the $\epsilon$-transitions into the same state (as it would get collapsed anyways).
                        \begin{align*}
                            \intertext{state 0:}
                            ~C'~ & \to \lrbt ~C, \$~ \\
                            ~C~ & \to \lrbt ~C i o, \$~ & \epsilon \text{ from first item} \\
                            ~C~ & \to \lrbt ~C i o, i~ & \epsilon \text{ from item above} \\
                            ~C~ & \to \lrbt ~i o, i~ & \epsilon \text{ from item above}\\
                            ~C~ & \to \lrbt ~i o, \$~ & \epsilon \text{ from first item}
                            \intertext{state 1:}
                            ~C'~ & \to ~C~ \lrbt ~, \$~ \\
                            ~C~ & \to ~C~ \lrbt ~i o, \$~ \\
                            ~C~ & \to ~C~ \lrbt ~i o, i~
                        ...
                        \begin{center}
                            \begin{tikzpicture}
                                \node[state, minimum size=1cm, initial] (0) at (0, 0) {$0$};
                                \node[state, minimum size=1cm, accepting] (1) at (2, 1) {$1$};
                                \node[state, minimum size=1cm] (2) at (4, 1) {$2$};
                                \node[state, minimum size=1cm] (3) at (6, 1) {$3$};
                                \node[state, minimum size=1cm] (4) at (2, -1) {$4$};
                                \node[state, minimum size=1cm] (5) at (4, -1) {$5$};
                                \draw
                                (0) edge[->, above] node{~C~} (1)
                                (0) edge[->, above] node{~i~} (4)
                                (1) edge[->, above] node{~i~} (2)
                                (2) edge[->, above] node{~o~} (3)
                                (4) edge[->, above] node{~o~} (5);
                            \end{tikzpicture}
                        \end{center}
                \end{enumerate}
        \subsection*{19th February 2020 \hfill Tutorial}
            \subsubsection*{LL Parsing}
                Note that we are able to use EBNF, which can make the grammar cleaner (without needing auxiliary rules).
                \begin{enumerate}[1.]
                    \itemsep0em
                    \item Write an LL(1) grammar with EBNF for boolean expressions consisting of the constants ~true~, ~false~, parentheses, and the operators ~and~, ~or~, ~not~, ensuring the following;
                        \begin{itemize}
                            \itemsep0em
                            \item precedence, from high to low, is ~not~, ~and~, ~or~
                            \item allow consecutive ~not~s
                        \end{itemize}
                        \begin{align*}
                            ~BoolExpr~ & \to ~AndExpr \{ 'or' AndExpr \}~ \\
                            ~AndExpr~ & \to ~NotExpr \{ 'or' NotExpr \}~ \\
                            ~NotExpr~ & \to ~'not' NotExpr~ \bnfsep ~Operand~ \\
                            ~Operand~ & \to ~'true'~ \bnfsep ~'false'~ \bnfsep ~'(' BoolExpr ')'~
                        \end{align*}
                \end{enumerate}
            \subsubsection*{Semantic Analysis}
                \begin{enumerate}[1.]
                    ...
                    \setcounter{enumi}{2}
                    \item
                        This question concerns the semantics of a return statement.
                        Note that we assume there exists some function that obtains the enclosing method for this statement.
                        The idea here is that we check the type of the return expression with the return type of the enclosing function, or check that it can accept a return without an expression (methods or "~void~" etc).
                        It's also important to remember that generally grammars describe statements in general, and do not provide context - therefore a body that shouldn't allow returns isn't captured by the syntax, and therefore must be checked in the semantic analysis stage.
                \end{enumerate}
            \subsubsection*{Runtime Organisation}
                \begin{enumerate}[1.]
                    \itemsep0em
                    \item For this question, just look at his slides about the memory organisation of classes (MLTs etc.)
                    \item If we are frequently modifying a string in a loop, it will often need a new memory block to be created to hold the new string (if it grows), requiring the old block to be freed, as well as copying memory - this can be slow and lead to heap fragmentation.
                \end{enumerate}
\end{document}
