    ...
    \section*{CO211 - Operating Systems}
        ...
        \subsection*{5th December 2019}
            ...
            \subsubsection*{Tutorial Questions}
                ...
                \end{enumerate}
        \subsection*{6th December 2019}
            \subsubsection*{Free Space Management}
                The file system now needs a way to manage the device's free space, requiring quick access to free blocks for allocation.
                Some methods are as follows;
                \begin{itemize}
                    \itemsep0em
                    \item \textbf{free list}
                        \medskip

                        This is simply a linked list containing the location of free blocks, and when new blocks are needed they are allocated from the beginning of this list.
                        When blocks are freed, they are appended to the end of this list.
                        This has low overhead to perform maintenance on the free list, however it can often lead to allocating non-contiguous blocks (unless we sort it, which then adds overhead).
                    \item \textbf{bitmap}
                        \medskip

                        The bitmap has one bit (in memory) for each disk block, where the $i^\text{th}$ bit corresponds to the $i^\text{th}$ block on the disk.
                        This bit is set to 1 if the bit is free, otherwise it is 0 if it is allocated.
                        As this is a very compact representation, we can load the entire bitmap into memory, and can therefore quickly determine available contiguous blocks (since this would be done in memory).
                \end{itemize}
            \subsubsection*{File System Layout}
                ...
                \begin{itemize}
                    ...
                    \item superblock
                        ...
                        This contains metadata for the file system, such as the number if inodes, the number of data blocks, the start of the bitmaps, the first data block, the block size, and the maximum file size.
                    \item free inode bitmap
                    \item free block (zone) bitmap
                    \item data and inode blocks
                        \medskip

                        We want the inodes to be spread out, as we don't want any hotspots.
                        This also gives us the benefit of access locality.
                \end{itemize}
                Directories aid with file organisation, as well as ensuring the uniqueness of names.
                Most operating systems give a hierarchical file system (tree structure), with a root directory.
                The directory system calls in Unix/Linux are as follows;
                \begin{center}
                    \begin{tabular}{ll}
                        system call & description \\
                        \hline
                        ...
                        ~s = closedir(path)~ & close directory \\
                        ~dirent = readdir(dir)~ & read one entry from directory \\
                        ~rewinddir(dir)~ & rewind directory to re-read
                    \end{tabular}
                \end{center}
                When we look up a file, it starts from the root directory (which is in a well-known inode).
                The next directory is found in the listing for the root directory (which contains which inode refers to the directory), and then that inode is checked.
                The block that this directory is on is found in the inode, and that block is then read.
                This continues until we get to the last directory in the path, and locate the file's inode in the directory block, and read the file's block from that inode to get the location on disk.
                As we are traversing through inodes, it also contains the access permissions (which is then used to verify the user is allowed to access those files).
                \begin{center}
                    \begin{tikzpicture}
                        \node at (-1, -0.5) {directory};
                        \node at (1, -0.5) {\shortstack{directory\\entry 1}};
                        \node at (3, -0.5) {\shortstack{directory\\entry 2}};
                        \node at (5, -0.5) {\shortstack{directory\\entry 3}};
                        \node at (7, -0.5) {\shortstack{directory\\entry 4}};
                        \node at (11, -0.5) {\shortstack{directory\\entry $n$}};
                        \draw
                        (0, 0) -- (8, 0) -- (8, -1) -- (0, -1) -- cycle
                        (2, 0) -- (2, -1)
                        (4, 0) -- (4, -1)
                        (6, 0) -- (6, -1)
                        (8, 0) edge[dashed] (10, 0)
                        (8, -1) edge[dashed] (10, -1)
                        (10, 0) -- (12, 0) -- (12, -1) -- (10, -1) -- cycle;
                    \end{tikzpicture}
                \end{center}
                ...
                \end{lstlisting}
            \subsubsection*{Links}
                Links can be thought of as pointers from one part of a file system tree to another part of the same file system tree.
                Hard links are references to the inode of a file (can be with a different name) - often unsupported for directories as it can cause a cycle in what would otherwise be a directory tree.
                Symbolic (soft) are a special type of file that reference the full pathname of another file or directory (this is ignored in a full file system traversal).
                \medskip

                Recall that the file metadata contained the number of (hard) links.
                This is used for the deletion of files, as we are essentially performing reference counting.
                When a file is deleted, we decrement this, and actually free the data (deallocate the inode) when there are no other hard links to it.
                This means that a hard link can never be "dangling" as it will always point to an inode, whereas a soft link can be, since the path it is pointing to may be deleted.
            \subsubsection*{Mounting}
                The mounting operation allows for multiple file systems to be combined into a single namespace.
                Soft links to files in mounted file systems are supported, but hard links are not supported as not all file systems use inodes.
                The directory in the native file system that is assigned to the root of the mounted file system is known as the \textbf{mount point}.
                The file system manages mounted directories with mount tables, which contain information about the location of mount points and devices (when a mount point is encountered, the mount table is used to determine the device and type of the mounted file system).
                \begin{center}
                    \hfill
                    \begin{tikzpicture}[x=2cm]
                        \node at (0, 0) {file system A};
                        \node (r) at (0, -1) {~/~};
                        \node (ru) at (-1, -2) {~usr/~};
                        \node (rh) at (0, -2) {~home/~};
                        \node (rm) at (1, -2) {~mnt/~};
                        \node (rmx) at (0.5, -3) {~xfs/~};
                        \node (rmn) at (1.5, -3) {~newfs/~};
                        \node (rmnf) at (0.5, -4) {~foo~};
                        \node (rmnb) at (1.5, -4) {~bar~};
                        \node (rmnd) at (2.5, -4) {~dir/~};
                    ...
                    \hfill
                    \begin{tikzpicture}[x=2cm]
                        \node at (0, 0) {file system B};
                        \node (r) at (0, -1) {~/~};
                        \node (ru) at (-1, -2) {~usr/~};
                        \node (rh) at (0, -2) {~home/~};
                        \node (rm) at (1, -2) {~mnt/~};
                        \node (rub) at (-1, -3) {~bin/~};
                        \node at (0, -4) {\phantom{~dir/~}};
                    ...
                    \hfill \phantom{}
                \end{center}
                The trees above are from before mounting, and the tree below is after file system B is mounted at ~/mnt/newfs~ in file system A.
                Note that the content originally at the mount point is no longer accessible.
                \begin{center}
                    \begin{tikzpicture}[x=2cm]
                        \node at (0, 0) {file system A};
                        \node (r) at (0, -1) {~/~};
                        ...
                        \node (rmx) at (0.5, -3) {~xfs/~};
                        \node (rmn) at (1.5, -3) {~newfs/~};
                        \node (rmnu) at (0.5, -4) {~usr/~};
                        \node (rmnub) at (0.5, -5) {~bin/~};
                        \node (rmnh) at (1.5, -4) {~home/~};
                        \node (rmnm) at (2.5, -4) {~mnt/~};
                    ...
                    \end{tikzpicture}
                \end{center}
            \subsubsection*{Linux ext2fs}
                The goal is a general purpose file system that gives high performance.
                It has typical block sizes of 1024, 2048, 4096, or 8192 bytes.
                There is also an additional safety process that reserves 5\% of blocks for root.
                For example, if a malicious or errant user process consumes all disk space, daemons such as loggers can no longer write to the disk.
                \medskip

                The inodes in ext2 consist of 15 pointers; the first 12 are direct pointers to 12 data blocks, followed by an indirect pointer, a doubly-indirect pointer, and then finally a triply indirect pointer.
                This provides fast access to small files, but also supports very large files.
                \medskip

                As we tend to want to use contiguous blocks, ext2fs has the addition of block groups, which are clusters of contiguous blocks.
                The file system attempts to store related data in the same block group, thus reducing seek time for accessing related data - this encourages locality of allocation.
                This looks similar to a self contained file system;
                \begin{itemize}
                    \itemsep0em
                    \item superblock \hfill contains redundant copies of superblock - limits data loss to some block groups
                    \item group descriptors \hfill block numbers for the following data
                    \item block allocation bitmap \hfill blocks used within groups
                    \item inode allocation bitmap \hfill which inodes have already been allocated
                    \item inode table \hfill where the inodes are located on the blocks
                    \item data blocks
                \end{itemize}
                The file system can then be considered as a sequence of these block groups.
        \subsection*{11th December 2019}
            \subsubsection*{Tutorial Questions}
                \begin{enumerate}[1.]
                    \itemsep0em
                    ...
                    \item
                        ...
                        \begin{itemize}
                            \itemsep0em
                            \item sharing virtual memory \hfill OS page protection
                            \item sharing processor cores \hfill context switches, interrupts
                            \item sharing files \hfill access control lists, encryption
                        \end{itemize}
                \end{enumerate}
            \subsubsection*{Security Goals}
                Our goals are generally to prevent unauthorised access to the system, and permit authorised sharing of resources.
                We care about data confidentiality (sensitive data is not stolen), data integrity (data is not altered or destroyed), and system availability (denial of service).
                \begin{center}
                    \begin{minipage}[t]{0.485\textwidth}
                        security policy (what security is provided)
                        \begin{itemize}
                            \itemsep0em
                            \item what is protected
                            \item who has access
                            \item what access is permitted
                        \end{itemize}
                    \end{minipage}
                    \hfill
                    \begin{minipage}[t]{0.485\textwidth}
                        ...
                        \begin{itemize}
                            \itemsep0em
                            \item how to implement security policy
                            \item same mechanisms can support different policies
                        \end{itemize}
                    \end{minipage}
                \end{center}
                ...
                \begin{itemize}
                    \itemsep0em
                    \item \textbf{people security} \hfill insider, social engineering attacks
                        \medskip

                        Employees need privileges to perform duties, but can abuse those privileges for gain.
                        Social engineering is another issue, where an attack may try to convince an individual to reveal sensitive details - furthermore, people may have incorrect expectations for security.
                        Another issue is working around security measures - for example forcing users to change passwords may lead to weak passwords.
                        \medskip

                        In general, this focuses on organisational policies, and how we handle mechanisms.
                    \item \textbf{hardware security} \hfill physically stealing disks
                        \medskip

                        With physical access, they can bypass security mechanisms much more easily - for example by physically removing a disk and reading the contents (hence full disk encryption may be required).
                        Network traffic can also be sniffed, if it is unencrypted.
                        Hardware implementations can also be exploited (such as Spectre or Meltdown).
                    \item \textbf{software security} \hfill exploiting bugs to gain permissions
                        \medskip

                        Bugs may allow an attacker to compromise a system.
                        These tend to exploit buffer overflows, integer overflows, and format string vulnerabilities.
                        This is more prevalent in memory unsafe languages such as C, which is what operating systems are generally written in.
                \end{itemize}
            \subsubsection*{Access Control}
                This is implemented by the OS, and typically carried out in two parts;
                \begin{itemize}
                    \itemsep0em
                    \item \textbf{authentication} \hfill verify identity of users
                        \medskip

                        The verification of identity of principal (generic term for user or process) is based on;
                        \begin{itemize}
                            \itemsep0em
                            \item personal characteristics (such as finger prints, retina patterns)
                                \medskip
                                This can suffer from high equipment cost, as well as false positives or negatives.
                            \item possessions (based on physical possessions such as RFID cards)
                                \medskip

                                This can ensure physical security, however it is expensive and impersonation attacks can happen if the item is lost.
                                Typically combined as part of two factor authentication for additional security.
                            \item knowledge (such as password)
                                \medskip

                                This is very cheap to implement, however dictionary attacks are able to find insecure passwords.
                                Furthermore, password reuse is also a big issue.
                                \medskip

                                Passwords should never be stored in plain text, as it is very vulnerable to data theft or disclosure through system administrators.
                                Modern systems store encrypted versions of passwords, computed with a one-way cryptographic hash function.
                                \medskip

                                This continues with the flaws of hashing due to rainbow tables and computational power, as well as the use of salting.
                        \end{itemize}
                    \item \textbf{authorisation} \hfill allow users to perform actions only when authorised
                        \medskip

                        The default decision (when a policy cannot be found) depends on the type of system, default to no access for a more security focused system.
                        When designing policies, the Principle of Least Privilege (PoLP) should be followed, which gives the user the minimum rights for a task.
                        This ensures that the minimum amount of damage is done if that particular user is compromised - however more rights are given for convenience.
                \end{itemize}
            \subsubsection*{Authorisation}
                A protection domain is a set of access rights, which is defined as a set of objects and the operations permitted on them.
                A principal executing in domain $D$ has access rights specified by $D$.
                This can be written as an access control matrix, where the rows represent principals (users, user groups), and the columns represent target objects (files, devices, processes);
                \begin{center}
                    ...
                    \end{tabular}
                \end{center}
                However, this is expensive to implement as a 2D array.
                We have two options;
                \begin{itemize}
                    \itemsep0em
                    \item \textbf{access-control lists (ACLs)} \hfill taking a column of the matrix
                        \medskip

                        This is stored with each object, and store it as part of the file's metadata (in the inode).
                    \item \textbf{capabilities} \hfill taking a row of the matrix
                        \medskip

                        Possession of a capability gives the right to perform the operations specified by it, similar to the possession of a key.
                        This is often not directly accessible by the user, and is therefore maintained by the OS, or an encrypted version is given to the user to ensure it has not been tampered with.
                \end{itemize}
                ...
                \begin{itemize}
                    ...
                    \item revocation
                        ...
                        This is easier with an ACL as we simply need to update the permission bits associated with a file.
                        On the other hand, this is harder to do with a capability, as it cannot be easily taken away.
                        Operating systems use revocation lists to handle this.
            ...
            \subsubsection*{UNIX / Linux}
                Users are the principals, and have a unique user id.
                The superuser, root, has a user id of 0, and can access any resource.
                As everything is a file in UNIX (and therefore accessed under the same access control mechanism), there are three types of operations; (R)ead, (W)rite, and e(X)ecute.
                Each user can belong to one or more groups, but each file can only belong to one group.
                Recall how running ~ls -l~ would give a line that started with the following;
                \begin{center}
                    \violet{~-~}\blue{~rw-~}\red{~r--~}\textcolor{teal}{~r--~}
                \end{center}
                \begin{itemize}
                    \itemsep0em
                    \item \violet{~-~} \hfill file type
                    \item \blue{~rw-~} \hfill access rights of file owner
                    \item \red{~r--~} \hfill access rights of group members
                    \item \textcolor{teal}{~r--~} \hfill access writes of everybody else
                \end{itemize}
                ...
                \begin{center}
                    \begin{minipage}[t]{0.485\textwidth}
                        ...
                        \begin{itemize}
                            \itemsep0em
                            \item ~-~ \hfill regular file
                            \item ~d~ \hfill directory
                            \item ~b~ \hfill block file
                            \item ~c~ \hfill character file
                            \item ~l~ \hfill symbolic link
                            \item ~p~ \hfill pipe
                            \item ~s~ \hfill socket
                        \end{itemize}
                    \end{minipage}
                    ...
                    \begin{minipage}[t]{0.485\textwidth}
                        ...
                        \begin{itemize}
                            ...
                            \item ~r~ \hfill read
                            \item ~w~ \hfill write
                            \item ~x~ \hfill execute
                            \item ~s~ \hfill setuid, setgid
                            \item ~t~ \hfill sticky bit
                        \end{itemize}
                    \end{minipage}
                \end{center}
                ...
                When a process is run, it inherits the access permissions of the user launching the process.
                It is essentially running on your behalf.
                Running ~passwd~ is a privileged operation, as only root should have access, however we are able to change our own passwords.
                By running the command ~ls -l \$(which passwd)~, we are able to see the following;
                \begin{center}
                    ~-rwsr-xr-x 1 root root 59640 Mar 22  2019 /usr/bin/passwd~
                \end{center}
                Note where the execute permission would be, there is an ~s~.
                This "set user id" bit runs it as the owner of the file, thus increasing privileges when using system programs.
                Each process has three IDs;
                \begin{itemize}
                    \itemsep0em
                    \item real UID \hfill ID of the user who started the process
                    \item effective UID \hfill this is used in access control checks
                    \item saved UID \hfill a saved ID to which the effective ID can be changed to
                        \subitem PoLP; even if it can execute as root, it uses a less privileged user if possible.
                \end{itemize}
            \subsubsection*{Tutorial Questions}
                \begin{enumerate}[1.]
                    \itemsep0em
                    \item
                        Represent the ownerships and permissions shown in this UNIX directory listing as an access control matrix.
                        ...
                        \begin{lstlisting}
                            -rw-r--r-- 3 b    systems  ...  .emacs
                            -rwxr-xr-x 3 b    users    ...  os.pptx
                            -rw-rw---- 1 b    systems  ...  notes.txt
                            -rw-r----- 3 b    users    ...  index.html
                        \end{lstlisting}
                        \begin{center}
                            ...
                            \end{tabular}
                        \end{center}
                    \item Why would ~setuid~ programs need to drop privileges?
                        \medskip

                        Only privileged tasks should be performed with elevated privileges to reduce the attack surface.
                        This is anther instance of the principle of least privilege.
                    \item
                        ...
                        Since all users have write permissions, the contents can be changed to anything (including malicious code).
                        Due to the ~s~ bit being set for execution, every execution of this process will run as root, thus allowing anyone to execute instructions as the superuser.
                        \medskip

                        Therefore it is important to ensure that access to writing ~setuid~ binaries is extremely limited.
                \end{enumerate}
            \subsubsection*{Richer Access Control Models}
                Discretionary Access Control (DAC) models allow for principals to determine who may access their objects.
                On the other hand, there is Mandatory Access Control (MAC) models, which has a privileged entity set access control rights for files and resources.
                The latter is much more secure (which is why this is used more in the web / cloud context - it is better for preventing data leakage), however the former is much more flexible.
                \medskip

                The \textbf{Bell-LaPadula} model is an example of MAC.
                We maintain the idea of objects and principals, and they each have an assigned security level (unclassified, confidential, top secret, etc).
                This enforces two rules;
                \begin{itemize}
                    \itemsep0em
                    \item \textbf{the simple security property}
                        \medskip

                        A process running at security level $k$ can only read objects at its level or lower.
                    \item \textbf{the * property}
                        \medskip

                        A process running at security level $k$ can write only objects at its level or higher.
                \end{itemize}
                This means that accidental disclosure cannot occur, as something from a higher level cannot be written down into a lower level, thus constricting the "flow" of information.
                This maintains confidentiality, but not integrity - which is what the next model does.
                \medskip

                The \textbf{Biba} model guarantees integrity in the following two rules;
                \begin{itemize}
                    \itemsep0em
                    \item \textbf{the simple integrity principle}
                        \medskip

                        A process running at security level $k$ can write only objects at its level or lower (no write up).
                    \item \textbf{the integrity * property}
                        \medskip

                        A process running at security level $k$ can read only objects at its level or higher (no read down).
                \end{itemize}
                This mirrors the previous model, in the sense that it guarantees integrity (we regard a higher level as being higher integrity).
                We can read data from a higher level, and write it down, which maintains integrity, but not read data with worse integrity (lower level), and write it up.
            \subsubsection*{Design Principles for Security}
                One of the mistakes early operating systems made was not including security mechanisms, as they simply assumed all users are trusted and malicious users didn't exist.
                Adding security mechanisms to an operating system is difficult.
                \begin{itemize}
                    \itemsep0em
                    \item give each process the least privilege possible \hfill default should be no privilege
                    \item protection mechanism should be simple and uniform
                        \subitem complexity can lead to bugs, or the user not understanding (thus incorrect policies)
                    \item psychologically acceptable \hfill usability
                    \item system design should be public \hfill security through obscurity is a bad idea
                \end{itemize}
\end{document}
