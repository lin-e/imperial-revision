    ...
    \section*{Advanced Computer Architecture \hfill (60001)}
        ...
        \subsection*{Chapter 10}
            ...
            \subsubsection*{Cache Coherency Protocols}
                This mainly looks at the `snooping' family of cache coherency protocols, which works for a modest number of cores.
                This is due to the reliance on some interconnect between the processors, allowing every processor to see all the traffic on the network.
                \medskip

                Consider the following example;
                \begin{center}
                    \begin{tikzpicture}[y=0.67cm]
                        \draw
                        (5.5, 3) -- (5.5, 2)
                        (4, 4) -- (7, 4) -- (7, 3) -- (4, 3) -- cycle
                        (0, 2) -- (11, 2) -- (11, 1) -- (0, 1) -- cycle;
                        \foreach \x in {0, 4, 8} {
                            \begin{scope}[shift={(\x, 0)}]
                                ...
                                \node at (1.5, -0.5) {~L2 cache~};
                                \node at (1.5, -2.5) {~L1 cache~};
                                \node at (1.5, -4.5) {~CPU~};
                                \draw
                                (1.5, 0) -- (1.5, 1)
                                (1.5, -2) -- (1.5, -1)
                        ...
                        \node at (1.5, -5.5) {processor 0};
                        \node at (5.5, -5.5) {processor 1};
                        \node at (9.5, -5.5) {processor 2};
                        \node at (5.5, 3.5) {~main memory~};
                        \node at (5.5, 1.5) {~interconnection network~};
                    \end{tikzpicture}
                \end{center}
                Suppose there is some memory location ~x~ that sits in main memory.
                First processor 0 loads from memory location ~x~; this is allocated in both the ~L2~ and ~L1~ cache for processor 0.
                Then processor 1 loads from the same location, which becomes allocated in both caches as well (now there are a total of 5 copies).
                Now assume that processor 0 stores to location ~x~; thus updating the ~L2~ and ~L1~ caches (assuming there's some writing back); note that the main memory isn't updated.
                Whether we actually write back or not doesn't make much difference to the fundamental problem; processor 1 doesn't know what's happened.
                Suppose processor 2 also reads from ~x~ - it needs to know whether to get ~x~ from main memory (occasionally, this can be the only source), processor 0, or processor 1.
                This raises two issues;
                \begin{itemize}
                    \itemsep0em
                    \item where do we find the latest version of the cache line
                    \item when can we use the cached copy, and when do we have to look for a more updated version
                \end{itemize}
                ...
                When we observe the behaviour of another core by watching its issued loads and stores, we should see something consistent with its program order.
                This may not happen as writes / reads may be reordered in some way.
                \medskip

                We don't want remote copies to be indefinitely out of date; one approach is to update whoever has a copy of the address once a store is executed (with an update message).
                To do this; we'd either need to broadcast every store to every remote cache, keep a list of which remote caches may hold the cache line, or at note that if there are any remote copies of this line (a shared bit).
                There are a number of problems with updates.
                One issue is that if the cache line is several words long, we may need to do a broadcast for each word in the line.
                We may also update processors that don't care about the new writes; it would be nice to have exclusive access to the cache line, to avoid broadcasting updates.
                \medskip

                Invalidation is a more popular plan; instead of updating remote cache lines, they are invalidated when a store occurs.
                After the first write to a cache line, we can be sure there are no remote copies (since they are all invalidated), meaning subsequent writes don't need communication.
                However, if another processor requests a copy, we will know more copies exist and require invalidation if we do another update.
                This is often better than updates, however if the processors really need the new data quickly, it forces them to suffer a read miss.
                Each cache line will need some additional state maintained, to exploit this.
                \medskip
                    ...
                    \begin{tikzpicture}[y=0.67cm]
                        ...
                        \foreach \x in {0, 4, 8} {
                            \begin{scope}[shift={(\x, 0)}]
                                ...
                                \node[violet] at (1.5, -0.5) {~SCC~};
                                \node at (1.5, -2.5) {~L2 cache~};
                                \node at (1.5, -4.5) {~L1 cache~};
                                \node at (1.5, -6.5) {~CPU~};
                                \draw
                                (1.5, 0) -- (1.5, 1)
                                (1.5, -2) -- (1.5, -1)
                        ...
                        \node at (1.5, -7.5) {processor 0};
                        \node at (5.5, -7.5) {processor 1};
                        \node at (9.5, -7.5) {processor 2};
                        \node at (5.5, 3.5) {~main memory~};
                        \node at (5.5, 1.5) {~interconnection network~};
                    \end{tikzpicture}
                \end{center}
                The snooping cache controller has to monitor all bus transactions and check them against its caches' tags.
                For example, if processor 0 was to invalidate address ~x~, processor 1's SCC would notice the presence of address ~x~ in its caches.
                Similarly, when processor 2 requests address ~x~, a read request is broadcasted on the bus.
                If another processor has a dirty copy of the data, it is important for it to supply this data (in this example, processor 0's copy is more recent than the one in main memory).
                The SCC therefore needs to monitor for read requests as well.
                \medskip
                ...
                \begin{itemize}
                    \itemsep0em
                    \item invalid \hfill
                    \item valid \hfill clean, potentially shared, unowned
                        \subitem The processor does not have the right to write to that data, nor is it the processor's responsibility to supply it on a request
                    \item shared-dirty \hfill modified, possibly shared, owned
                        \subitem From the dirty state, if someone else tries to read it, it supplies it and becomes shared-dirty
                    \item dirty \hfill modified, only copy, owned
                        \subitem This has been written to and an invalidation has been broadcast (hence only copy and exclusive access) - subsequent writes don't need another invalidation
                \end{itemize}
                ...
                \begin{itemize}
                    ...
                    \item \textbf{read miss}
                        ...
                        On a read miss, we broadcast the request on the bus.
                        If another cache line has the line in either a shared-dirty or dirty state, it supplies it and sets its own state to shared-dirty, our copy is set to valid.
                        Otherwise, the line comes from memory, set to valid.
                    \item \textbf{write hit}
                        \smallskip

                        On a write hit, nothing happens if the line is in a dirty state (already invalidated all other copies, no further invalidation is required).
                        However, if the state is valid or shared-dirty, an invalidation is performed and the local state is set to dirty.
                        This is done to gain exclusive access.
                    \item \textbf{write miss}
                        \smallskip

                        The line comes from the owner (either main memory or another processor, as with the read miss case).
                        All other copies are invalidated (set to invalid) and the line in the requesting (local) cache is set to dirty.
                \end{itemize}
                The cache controller will see all the memory traffic of all of the cores.
                Transitions between states are triggered either by the bus (invalidate, write miss, read miss) or the CPU (read hit, read miss, write hit, write miss).
                For every transaction on the bus, the cache controller needs to look up the directory (cache line state) information for a specified address.
                The states don't handle what happens in a cache displacement (see textbook). % lol no
                \medskip

                The dirty bit records whether remote copies exist (invalidation is done by broadcasting messages on the bus).
                It also handles where to get updated data with a read miss request on the bus.
                \medskip

                Since every bus transaction checks cache tags, there could be contention between what the SCC wants to do with tags and the CPU.
                One solution is to duplicate the tags, to give the SCC parallel access to the cache state.
                Another solution is to use multi-level inclusion (if everything in L1 is in L2, then we can use L2 to filter invalidations - only have to check L1 if L2 tag matches).
                Alternatives for cache inclusivity is a snoop filter.
            \subsubsection*{Synchronisation and Memory Models}
                We need to synchronise in order to know when it's safe for different processes to used shared data.
                In order to implement locking, we need a form of uninterruptible primitive.
                We need a way to perform both a read and write in an atomic indivisible operation.
                Once we can build one of these primitives, we can build on top of it.
                We want the non-contended path to be fast, and efficient in the case where there is high contention.
                The distribution should also be fair.
                \medskip

                All of the following operations require us to do a load and store in a single operation - however, note that no instruction has more than one memory access for a RISC instruction set;
                \begin{itemize}
                    \itemsep0em
                    \item test-and-set
                    \item fetch-and-increment
                    \item atomic exchange
                \end{itemize}
                This is somewhat feasible in a single-core machine, by locking the memory bus to ensure nothing else can be scheduled in between.
                This is also possible if done in the memory (not on the CPU).
                \medskip

                Note that the L1 cache of a GPU doesn't run a cache coherence protocol; it is flushed once a kernel is launched.
                Load instructions can be buffered in the cache, as well as stores (which may eventually get to main memory), but there is no way for other processors and their caches to know data has been updated.
                Atomic operations have to be handled in the L2 cache.
                \medskip

                In a multi-core CPU machine, the idea is to use two instructions; load liked (or load locked), and store conditional.
                The load linked returns the initial value and stores the address; however, the store conditional only completes if no invalidations for that address have been received since the load completed (doesn't update memory if it fails, but rather returns a 0 if it fails or 1 if it succeeds).
                For example, in atomic exchange (if we are interrupted for any reason, retry);
                \begin{lstlisting}
                ...
                \end{lstlisting}
                Another example is fetch and increment;
                \begin{lstlisting}
                    try: ll   R2,0(R1) ; load linked
                         addi R2,R2,#1 ; increment (register to register is fine)
                         sc   R2,0(R1) ; store conditional
                         beqz R2,try   ; branch if the store fails (R2 = 0)
                \end{lstlisting}
                This is done by remembering the load address and checking it against any invalidations.
                Some processors extend this idea to allow for transactions.
                \medskip

                This can be used to implement a spin lock, where the processor continues to acquire a lock and loops if it fails.
                Note here we load a 1 into memory, and exchange it with the lock; if it fails (~R2~ isn't zero), we try again;
                \begin{lstlisting}
                    start:  li   R2,#1
                    lockit: EXCH R2,0(R1)  ; atomic exchange (from above)
                            bnez R2,lockit ; loop if lock is already acquired
                \end{lstlisting}
                We need the cache line where the lock falls to be in a dirty state.
                If we have multiple processors spinning on this lock, then they will compete to have the cache line in the dirty state, leading to a constant exchange of invalidations and coherency messages; generating significant bus traffic.
                A solution to this is to read the variable and then try the exchange if it changes;
                \begin{lstlisting}
                    try   : li   R2,#1
                    lockit: lw   R3,0(R1)  ; load var
                            bnez R3,lockit ; if not free, spin
                            EXCH R2,0(R1)  ; atomic exchange
                            bnez R2, try   ; try again if already locked
                \end{lstlisting}
                This is an improvement as they only compete once the lock is released (rather than all the time).
                However; the processor that wins this may be deterministic; which is unfair.
                \medskip

                A ticket lock starts with the current ticket and the active ticket being zero.
                This explicitly hands off access to the next in line;
                \begin{lstlisting}
                    ...
                    ticketLock_acquire(int *next_ticket, int *now_serving) {
                        ...
                        while (*now_serving != my_ticket) { } // spin lock
                    }
                ...
                \end{lstlisting}
                The number of caches misses per acquisition is proportional to the number of processors / threads that are waiting on the lock.
                The ticket lock can experience a performance collapse with more cores.
                \medskip
                ...
                \begin{lstlisting}
                    ...
                    L2: if (A == 0) ...
                \end{lstlisting}
                Intuitively, it seems that both conditions can't be true.
                However, if we do not wait for the invalidation (as it needs to be acknowledged) and allow the processor to continue, we can observe the thread in an inconsistent order.
                If we wait for the acknowledgement, we can get sequential consistency (where our reasoning holds); therefore we need to delay all memory accesses until all invalidations are complete.
                \medskip

                Weak consistency can be faster however.
                If we aren't able to get sequential consistency, we need some sort of consistency still; instead of a total ordering, the accesses may be ordered only at synchronisation operations (the points where we end up waiting for invalidations to be completed).
                Different architectures have subtly different memory consistency models.
            \subsubsection*{Scalable Cache Coherency Protocols}
                For larger systems, directory based cache coherency protocols can be used to reduce the amount of broadcasting in the system, giving desirable scalability properties.
                The snooping cache coherency protocol relies on busses to broadcast invalidations and read requests, as well as to establish a global ordering on all events in the system.
                However, as every access is monitored, the bus becomes a bottleneck with many processors; snooping will no longer work and a more general interconnection network is required.
                \medskip

                Main memory will also be distributed around the system, leading to NUMA (non-uniform memory architecture), where some DRAM is closer to some cores than others.
                Ideally, when a processor allocates memory, it allocates with its local DRAM, requiring the OS to know where the local DRAM is and where allocation should be made.
                Copies of remote data are also cached.
                \medskip

                Directories are how we represent what we know about where the data lives.
                Consider a system with separate memory per processor, with local / remote access via the memory controller.
                There's a directory per cache, tracking the state of every block in the cache (such as which caches has a copy of a block, the state, etc.).
                We need to consider whether to keep information per memory block or per cache block, as well as preventing the directory from being a bottleneck.
                \medskip

                An example of this is \textit{Sun}'s \textit{S3MP}.
                Each processor has a cache controller, some memory, and a CPU.
                Directory information was included with every cache line stored in memory, as well as with every cache line in the cache.
                The cache consists of a tag, a dirty bit, and a pointer (which is also in main memory).
                The pointer is used to implement a linked list that connects all caches which hold a copy of this data.
                As processors request read copies of the data and cache them locally, a linked list is constructed (it may start as a null pointer).
                This allows an invalidation message to be sent which visits each of these caches and invalidates the copy.
                \medskip

                Each cache line also has a home node, which is a unique place where it will live if it lives in DRAM.
                We can tell what home node the particular address is in by looking at the physical address (after address translation).
                All the activity with respect to a specific cache line can be handled by that home node.
                \medskip

                See the example on the slides for this.
                Node 2 makes a request and node 5 (home) updates it, creating a pointer.
                The next case is when another processor (node 7) has the data.
                Node 2 asks node 5 as it's the home node, but the read request is forwarded to node 7 which has the data, which then shares it to node 2.
                It also then updates its sharing pointer to point to node 2 (before it was pointing to node 4), and tells node 2 to then point to node 4.
                \medskip

                When a write occurs, the node makes a request to the home node, which sets a lock on the cache line, preventing simultaneous transactions.
                An invalidation message is passed to the head of the sharing list, and all the way down (including the writing node, even if it has a shared copy).
                Once it gets to the final node, it sends two messages; one to unlock it (sent to home) and another to the writing node, saying it's done.
                \medskip

                For a displacement to occur, the displacing node sends an `unhook' request to the home node.
                This locks the cache line in the home.
                The home node then hooks an unhook forwarding message to the head of the chain, which gets forwarded down the chain up to the node \textbf{prior to the displacing node}.
                The node requests the pointer from the displacing node which gives it and changes its pointer to the displacing node's successor.
                An unlock is then sent from the predecessor node.
                \medskip

                For a CPU to find a valid copy, it needs to do the following;
                \begin{enumerate}
                    \itemsep0em
                    \item translate virtual address to physical
                    \item identify home node from physical address' high order bits
                    \item home node is where the DRAM for an address resides
                    \item current valid copy may not be at the home node (may be in another CPU's cache), but it holds pointer to the sharing chain, so it can find it
                \end{enumerate}
                This has strong sequential consistency (recent designs have weaker models).
                It uses a singly-linked chain, which means there's less metadata.
                A disadvantage occurs with long chains (when the data is widely-shared), with operations begin time consuming with many sequential messages.
                However, simulations showed that invalidation over widely shared data is rare (read-only is fine).
                On average, they were right; but the worst case behaviour was quite poor.
                \medskip

                Other approaches included a doubly-linked sharing list, or a 64-bit sharing list (which has a bit for each of the remote processors where the data might be).
                With the latter approach, a bit may represent multiple processors (where an invalidation is broadcast to all processors under that bit, for example 4 processors with \textit{SGI Origin 2000}'s 256 CPUs).
                Multiple busses were also used, but was not as scalable (required some replication of tag arrays).
                \medskip

                Each processor in \textit{Intel Haswell} can come with up to 18 cores, with 9 cores in each ring, and 4 DRAM interfaces (2 per ring).
                The chip also had an interconnection interface, called the QPI interface.
                The chip was designed to be used in pairs on a dual socket system.
                In socket 1, there are nodes 0 and 1, and in socket 2 there are nodes 2 and 3, representing the rings.
                Each node is treated as a different NUMA subdomain, where each domain has uniform access to its main memory (and caches in the ring), and non-uniform access to others.
                Home agents in this machine has a similar role to home nodes from before.
                This is a hybrid between a snooping based machine and a directory based machine.
                Each cache line contains 2 bits of directory, which indicate whether the line is held in any other nodes (remote-invalid (no other copies in other nodes), snoop-all (potentially modified copy exists), or shared (multiple clean, consistent with main memory, exists)).
                \medskip

                On an L2 miss, the core sends a request to a cache agent on its node (based on a hash of the physical address).
                The agent looks after a block of LLC, and checks for a hit.
                If it has a hit, then the line will include directory bits.
                If it misses, it passes the request to the home agent, which is responsible for sending a read request or broadcasting invalidations.
                To do this, it may have more directory information.
                This directory information, for frequently exchanged lines, are cached in the home agent (8 bits, indicating which nodes may hold this).
\end{document}