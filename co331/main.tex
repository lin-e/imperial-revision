    ...
    \section*{CO331 - Network and Web Security \hfill (60015)}
        ...
        \subsection*{Week 4}
            ...
            \subsubsection*{PHP}
                ...
                \begin{lstlisting}
                    $\dollar$x = 0;
                    $\dollar$y = &$\dollar$x;       // $\dollar$x and $\dollar$y are now aliased
                    $\dollar$y = "hello";
                    echo $\dollar$x;       // prints "hello"
                \end{lstlisting}
                It also allows for dynamic variable names;
                \begin{lstlisting}
                    $\dollar$x = "y";
                    $\dollar$y = "hello";
                    echo $\dollar$$\dollar$x;       // prints "hello"
                ...
                \begin{lstlisting}
                    ...
                    $\dollar$x[] = "default";    // uses the default key 5 (first int greater than all keys)
                    echo $\dollar$x[5];          // prints "default"
                    echo current($\dollar$x);    // prints "bar"
                    next($\dollar$x);            // advances the pointer
                    echo current($\dollar$x);    // prints "baz"

                    $\dollar$GLOBALS["y"] = 42;  // 'super' global
                    echo $\dollar$y;             // prints 42
                \end{lstlisting}
                PHP also handles objects as expected;
                \begin{lstlisting}
                    $\dollar$obj -> x = 0;
                    var_dump($\dollar$obj); // prints the following (see next line)
                    > object(stdClass)#1 (1) { ["x"] => int(0) }
                \end{lstlisting}
                We can also define classes and inheritance;
                \begin{lstlisting}
                    class prnt {
                        private $\dollar$id = "foo";
                    ...
                    $\dollar$obj = new chld();
                    $\dollar$obj -> displayParent(); // prints "foo"
                \end{lstlisting}
                In arrays, there are subtle quirks in copying;
                \begin{lstlisting}
                    $\dollar$x = array(1, 2, 3);
                    $\dollar$y = $\dollar$x;
                    $\dollar$x[0] = "updated";
                    echo $\dollar$y[0];              // prints 1 (as expected)

                    $\dollar$x = array(1, 2, 3);
                    $\dollar$temp = &$\dollar$x[1];           // here we introduce sharing
                    $\dollar$y = $\dollar$x;                  // assigning normally
                    $\dollar$x[0] = "regular";       // update regular element
                    $\dollar$x[1] = "shared";        // update shared element

                    var_dump($\dollar$x);            // gives the following;
                    > array(3) {
                        [0] => string(7) "regular"
                        [1] => &string(6) "shared"
                        [2] => int(3) }

                    var_dump($\dollar$y);            // gives the following;
                    > array(3) {
                        [0] => int(1)
                        [1] => &string(6) "shared"
                        [2] => int(3) }
